...
def is_admin(user_id: int) -> bool:
    return user_id == SUPER_ADMIN_ID or user_id in TEAM_ADMINS.values()
...
async def show_admin_panel(message, user_id: int) -> None:
    team = admin_team(user_id)
    role = "ADMIN TỔNG" if user_id == SUPER_ADMIN_ID else f"ADMIN {team.upper()}"
    await message.reply_text(
        f"✅ ĐÃ ĐĂNG NHẬP QUYỀN {role}\n\n"
        "Các lệnh quản lý:\n"
        "/napxu TELEGRAM_ID SO_XU\n"
        "/truxu TELEGRAM_ID SO_XU\n"
        "/kiemtra TELEGRAM_ID"
    )
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # Tài khoản quản trị được nhận diện trực tiếp, không đi qua bước xin kích hoạt.
    if is_admin(user.id):
        set_approved(user.id)
        await show_admin_panel(update.message, user.id)
        return
...
async def consent(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(query.from_user.id):
        set_approved(query.from_user.id)
...
async def request_login(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_admin(user.id):
        set_approved(user.id)
        await show_admin_panel(query.message, user.id)
        return
    row = get_user(user.id)
    if is_approved(user.id):
...
async def next_round(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_approved(query.from_user.id):
        await query.message.reply_text("❌ Tài khoản chưa được kích hoạt.")
        return
    context.user_data.pop("cai", None)
...
async def run_check(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.message.reply_text(
        ...
        f"💰 Số dư: {balance} xu",
        reply_markup=InlineKeyboardMarkup([[
            InlineKeyboardButton("VÁN TIẾP THEO", callback_data="next_round"),
